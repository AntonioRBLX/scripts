	...
	if scriptactivated and not checkcaller() and LocalPlayer.Character then
		if configs.GunAimbot and tostring(self) == "ShootGun" and tostring(method) == "InvokeServer" then
			...
			if closest then
				...
				local _, aimpos = Aimbot:ComputePathAsync(attachment.WorldPosition,closest,90,0,nil,true,configs.Prediction,nil,true)
				attachment:Destroy()
		...
		elseif configs.KnifeAimbot and tostring(self) == "Throw" and tostring(method) == "FireServer" then
			...
			if closest then
				...
				local _, aimpos = Aimbot:ComputePathAsync(attachment.WorldPosition,closest,90,0,nil,true,configs.Prediction,nil,false)
				attachment:Destroy()
